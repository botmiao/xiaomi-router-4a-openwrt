...
def listener():
    ...
    srv.bind(('0.0.0.0',9999)); srv.listen(16); srv.settimeout(420)
    try:
        c,a=srv.accept()
        c.settimeout(400)
        try:
...
cmd=("( trap '' INT TERM HUP PIPE; /sbin/mtd -r write /tmp/fw.bin OS1 > /tmp/mtd.log ) | /usr/bin/telnet {m} 9999".format(m=myip))
...
print('mtd should be running now (immune to SIGINT)...',flush=True)
